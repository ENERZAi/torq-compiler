...
IREE_FLAG(
    string, torq_desc_data_dir, "",
    "Use existing Torq API descriptor dump (generated by the compiler) as template for the Torq "
    "API data dump"
);
...
IREE_FLAG(
    string, torq_dump_mem_data_dir, "", "Write Torq API data dump files to the specified directory"
);
IREE_FLAG(
    string, torq_dump_io_data_dir, "",
    "Dump input output buffers in binary form to the specified directory"
);
...
IREE_FLAG(
    string, torq_dump_buffers_dir, "",
    "Enable dumping intermediate buffers during execution to the specified directory"
)
IREE_FLAG(
    string, torq_hw_type, DEF_HW_TYPE, "Hardware type [sim, aws_fpga, soc_fpga, astra_machina]"
)
...
static std::string get_buffer_dump_dir(std::string executable_name, int job_id) {
    if (!FLAG_torq_dump_buffers_dir[0]) {
        ...
        return std::string(FLAG_torq_dump_buffers_dir) + "/" + executable_name + "/action" +
               std::to_string(job_id) + "/";
    }
}

static std::string get_mem_list_name(std::string executable_name, bool from_host, int job_id) {
    return get_mem_dump_data_dir(executable_name, job_id) + "tv." + (from_host ? "init" : "exit") +
           ".mem.lst";
}

static bool hex_file_write(
    string executable_name, string file_name, string type, int address, const void *data,
    size_t size, int job_id
) {
    ...
    fprintf(
        fp, "%s  0x%08x %10zu %3d  hex  %s\n", type.c_str(), address, size, 1, file_name.c_str()
    );
    fclose(fp);
    return true;
}

extern const iree_hal_executable_vtable_t iree_hal_torq_native_executable_vtable;
} // namespace
...
iree_status_t iree_hal_torq_native_executable_create(
    ...
    *out_executable = NULL;
    ...
    memcpy(
        executable->program, executable_params->executable_data.data,
    ...
    if (executable_params->constant_count > 0) {
        uint32_t *target_constants = (uint32_t *)((uint8_t *)executable + sizeof(*executable) +
                                                  executable_params->pipeline_layout_count *
                                                      sizeof(*executable->pipeline_layouts));
        memcpy(
            target_constants, executable_params->constants,
            executable_params->constant_count * sizeof(*executable_params->constants)
        );
    ...
    if (iree_status_is_ok(status)) {
        iree_hal_resource_initialize(
            &iree_hal_torq_native_executable_vtable, &executable->resource
        );
...
static void iree_hal_torq_native_executable_destroy(iree_hal_executable_t *base_executable) {
    iree_hal_torq_native_executable_t *executable =
        iree_hal_torq_native_executable_cast(base_executable);
    iree_allocator_t host_allocator = executable->allocator;
    IREE_TRACE_ZONE_BEGIN(z0);
...
iree_status_t sync_binding(
    int binding_id, iree_hal_torq_Binding_table_t binding,
    iree_hal_executable_dispatch_state_v0_t *state, TorqHw *torq, bool from_host,
    const string &executable_name, int job_id
) {
    ...
    if (bindingId >= state->binding_count) {
        return iree_make_status(
            IREE_STATUS_INVALID_ARGUMENT, "binding id %d out of range [0, %d)", bindingId,
            state->binding_count
        );
    }
    ...
    if (bindingOffset >= state->binding_lengths[bindingId]) {
        return iree_make_status(
            IREE_STATUS_INVALID_ARGUMENT, "binding offset %d out of range [0, %zu)", bindingOffset,
            state->binding_lengths[bindingId]
        );
    }

    if (bindingOffset + bindingSize > state->binding_lengths[bindingId]) {
        return iree_make_status(
            IREE_STATUS_INVALID_ARGUMENT, "binding size %d with offset %d out of range [0, %zu)",
            bindingSize, bindingOffset, state->binding_lengths[bindingId]
        );
    }
    ...
    if (FLAG_torq_dump_io_data_dir[0] && !io_str.empty()) {
        string data_file = "npu" + io_str + ".host_binding." + to_string(bindingId) +
                           ".torq_binding." + to_string(bindingId) + ".bin";

        LOGD << "Writing binary dump of binding " << bindingId << " to "
             << get_io_dump_data_dir(executable_name) << "/" << data_file;

        binary_file_write(
            get_io_dump_data_dir(executable_name) + data_file, hostAddress, bindingSize
        );
    }
    ...
    if (FLAG_torq_dump_mem_data_dir[0] && !io_str.empty()) {
        ...
        if (!hex_file_write(
                executable_name, file_name, type, bindingAddress, hostAddress, bindingSize, job_id
            )) {
...
static iree_status_t create_buffers_index(iree_hal_torq_native_executable_t *executable) {
    ...
    fp << "id;type;shape;strides;address;size;allocation_action;last_use_action;deallocation_"
          "action;allocation_location;last_use_location;deallocation_location\n";
    ...
    for (int i = 0; i < ns(BufferDebugInfo_vec_len(buffers)); i++) {
        ...
        std::string element_type_str =
            ns(ElementType_name(ns(BufferDebugInfo_element_type(buffer))));
        std::transform(
            element_type_str.begin(), element_type_str.end(), element_type_str.begin(), ::tolower
        );
        fp << element_type_str << ";";
...
iree_status_t create_dir(std::string dir_name) {
    ...
    if (ec && ec != std::errc::file_exists) {
        return iree_make_status(
            IREE_STATUS_INTERNAL, "failed to create directory %s: %s", dir_name.c_str(),
            ec.message().c_str()
        );
    }
    return iree_ok_status();
...
iree_status_t create_job_directory(std::string executable_name, int job_id) {
    ...
    if (FLAG_torq_dump_mem_data_dir[0]) {
        ...
        if (FLAG_torq_desc_data_dir[0]) {
            std::string template_dir = std::string(FLAG_torq_desc_data_dir) + "/" +
                                       executable_name + "/job" + std::to_string(job_id) + "/";
            ...
            if (filesystem::exists(template_dir)) {
                ...
                if (!filesystem::exists(mem_data_dir)) {
                    filesystem::copy(
                        template_dir, get_mem_dump_data_dir(executable_name, job_id),
...
static iree_status_t torq_execute_step_by_step(TorqHw *torq, int32_t code_entry, int32_t job_id) {
    ...
    if (patched_cmds.size() != cmds.size()) {
        LOGD << "Failed to patch code data, expected " << cmds.size() << " commands, but got "
             << patched_cmds.size();
...
static iree_status_t copy_to_hal_buffer(
    ...
    for (size_t i = 0; i < total_elements; i++) {
        ...
        status = iree_hal_buffer_map_write(
            hal_buffer, i * element_size_bytes, src.data() + src_offset, element_size_bytes
        );
...
static iree_status_t dump_to_numpy(
    std::string filePath, iree_hal_torq_native_executable_t *executable,
    ...
    status = iree_hal_buffer_view_create(
        hal_buffer, shape.size(), shape.data(), element_type,
        IREE_HAL_ENCODING_TYPE_DENSE_ROW_MAJOR, executable->allocator, &buffer_view
    );
    ...
    status = iree_numpy_npy_save_ndarray(
        stream, IREE_NUMPY_NPY_SAVE_OPTION_DEFAULT, buffer_view, executable->allocator
    );
...
static void
dump_buffers(TorqHw *torq, int32_t action_id, iree_hal_torq_native_executable_t *executable) {
    ...
    if (ns(HostAction_params_type(action)) == ns(HostActionParams_StartNSSParams)) {
        ...
        if (!torq->wait(
                true, ns(StartNSSParams_starts_slice1(params)),
                ns(StartNSSParams_starts_slice2(params)), ns(StartNSSParams_starts_dma_in(params)),
    ...
    for (size_t i = 0; i < buffer_count; i++) {
        ...
        LOGD << "Dumping buffer " << bufferIndex << " at address " << address << " size " << size
             << " (data size " << bufferDataLengthBytes << ")";
        ...
        iree_status_t status =
            dump_to_numpy(file_path, executable, elementType, stridedData, shape, strides);
...
class ExecutionContext {
  ...
  public:
    ExecutionContext(
        TorqHw *torq, iree_hal_torq_native_executable_t *nativeExecutable, TorqEventLog *eventLog
    )
    ...
    iree_status_t run() {
        ...
        while (actionIndex < actionsCount) {
            ...
            if (ns(HostAction_location_is_present(action))) {
                LOGD << "Starting action " << actionIndex << " at location "
                     << ns(HostAction_location_get(action)) << ")";
            }
            ...
            if (ns(HostAction_location_is_present(action))) {
                LOGD << "Completed action " << actionIndex << " at location "
                     << ns(HostAction_location_get(action)) << ")";
            }
    ...
    int lastCompletedJobId() { return pendingHostCopies ? jobId : jobId - 1; }

  private:
    TorqHw *torq;
    iree_hal_torq_native_executable_t *nativeExecutable;
    bool pendingHostCopies{
        false
    }; // true if there are host copies pending and therefore bindings are not up to date yet
    int actionIndex;
    int jobId{0};
    ...
    std::string executableName() { return ns(ExecutableDef_executable_name(executableDef())); }
    ...
    iree_status_t loadHostCode() {
        ...
        if (!ofs.good()) {
            close(fd);
            return iree_make_status(
                IREE_STATUS_INTERNAL, "Failed to open temporary file for writing"
            );
        }
    ...
    iree_status_t
    processStartHostAction(ns(HostAction_table_t) action, ns(StartHostParams_table_t) params) {
        if (eventLog_)
            eventLog_->addEvent(
                Event{
                    std::chrono::steady_clock::now(), EventType::HOST_PROGRAM_START, actionIndex,
        ...
        auto hostFunction = (void (*)(
            iree_hal_executable_environment_v0_t *, iree_hal_executable_dispatch_state_v0_t *,
            iree_hal_executable_workgroup_state_v0_t *
        ))dlsym(hostCodeLibHandle, functionName.c_str());

        if (!hostFunction) {
            return iree_make_status(
                IREE_STATUS_INTERNAL, "Failed to find host function %s", functionName.c_str()
            );
        }
        ...
        for (int i = 0; i < flatbuffers_uint64_vec_len(args); i++) {
            ...
            torq->readXram(
                flatbuffers_uint64_vec_at(args, i), xramBuffer.size(), xramBuffer.data()
            );
        ...
        for (int i = 0; i < dispatch_state.binding_count; i++) {
            LOGD << "Argument " << i << ": " << std::hex << "0x"
                 << (uintptr_t)dispatch_state.binding_ptrs[i] << " xram: 0x"
                 << flatbuffers_uint64_vec_at(args, i) << std::dec
        ...
        for (int i = 0; i < flatbuffers_uint64_vec_len(args); i++) {
            torq->writeXram(
                flatbuffers_uint64_vec_at(args, i), arguments[i].size(), arguments[i].data()
            );
        }
        if (eventLog_)
            eventLog_->addEvent(
                Event{
                    std::chrono::steady_clock::now(), EventType::HOST_PROGRAM_END, actionIndex,
                    ns(HostAction_location_is_present(action)) ? ns(HostAction_location_get(action))
                                                               : ""
                }
            );
    ...
    iree_status_t
    processWaitHostAction(ns(HostAction_table_t) action, ns(WaitHostParams_table_t) params) {
        // host execution is synchronous for the moment so we don't need to do anything to wait
        return iree_ok_status();
    }

    iree_status_t
    processStartNSSAction(ns(HostAction_table_t) action, ns(StartNSSParams_table_t) params) {
        if (eventLog_)
            eventLog_->addEvent(
                Event{
                    std::chrono::steady_clock::now(), EventType::NSS_START, actionIndex,
                    ns(HostAction_location_is_present(action)) ? ns(HostAction_location_get(action))
                                                               : ""
                }
            );
        ...
        if (!torq->start(code_entry)) {
            return iree_make_status(
                IREE_STATUS_INTERNAL, "torq failed to start with code entry %d", code_entry
            );
        }
        if (eventLog_)
            eventLog_->addEvent(
                Event{
                    std::chrono::steady_clock::now(), EventType::NSS_START_END, actionIndex,
                    ns(HostAction_location_is_present(action)) ? ns(HostAction_location_get(action))
                                                               : ""
                }
            );
    ...
    iree_status_t
    processWaitNSSAction(ns(HostAction_table_t) action, ns(WaitNSSParams_table_t) params) {

        LOGD << "action: Waiting for NSS job " << jobId;
        if (eventLog_)
            eventLog_->addEvent(
                Event{
                    std::chrono::steady_clock::now(), EventType::NSS_WAIT_START, actionIndex,
                    ns(HostAction_location_is_present(action)) ? ns(HostAction_location_get(action))
                                                               : ""
                }
            );
        ...
        LOGD << "Job " << jobId << " Time since open (ms): " << t_since_open / 1.e3
             << " Time since start (us): " << static_cast<long>(t_since_start);
        ...
        if (eventLog_)
            eventLog_->addEvent(
                Event{
                    std::chrono::steady_clock::now(), EventType::NSS_WAIT_END, actionIndex,
                    ns(HostAction_location_is_present(action)) ? ns(HostAction_location_get(action))
                                                               : ""
                }
            );
    ...
    iree_status_t
    processHostCopyAction(ns(HostAction_table_t) action, ns(HostCopyParams_table_t) params) {
        if (eventLog_)
            eventLog_->addEvent(
                Event{
                    std::chrono::steady_clock::now(), EventType::HOST_COPY_START, actionIndex,
                    ns(HostAction_location_is_present(action)) ? ns(HostAction_location_get(action))
                                                               : ""
                }
            );
        ...
        auto copyElement = [&](int inputOffset, int outputOffset, int size) {
            ...
            if (FLAG_torq_dump_mem_data_dir[0]) {
                const string file_name = "host_copy_" + std::to_string(actionIndex) + "_" +
                                         std::to_string(outputAddress + outputOffset) + ".txt";
                ...
                if (outputType == ns(BufferType_LRAM)) {
                    ...
                    return iree_make_status(
                        ...
                        "unsupported output memory type when dumping descriptors"
                    );
                }
                ...
                if (!hex_file_write(
                        executableName, file_name, type + "load", outputAddress + outputOffset,
                        data.data(), data.size(), jobId
                    )) {
                ...
                if (jobId > 0) {
                    ...
                    if (!hex_file_write(
                            executableName, file_name, type + "save", outputAddress + outputOffset,
                            data.data(), data.size(), jobId - 1
                        )) {
                        return iree_make_status(
                            IREE_STATUS_INTERNAL, "failed to dump host copy data"
                        );
                    }
                }
            }
        ...
        std::function<iree_status_t(int, int, int)> copyDimension = [&](int dim, int inputOffset,
                                                                        int outputOffset) {
            if (dim == rank) {
                ...
                for (int i = 0; i < flatbuffers_uint32_vec_at(shape, dim); i++) {
                    auto ret = copyDimension(
                        dim + 1,
                        inputOffset + i * flatbuffers_uint32_vec_at(inputStridesBytes, dim),
        ...
        if (eventLog_)
            eventLog_->addEvent(
                Event{
                    std::chrono::steady_clock::now(), EventType::HOST_COPY_END, actionIndex,
                    ns(HostAction_location_is_present(action)) ? ns(HostAction_location_get(action))
                                                               : ""
                }
            );
    ...
    iree_status_t
    processAllocAction(ns(HostAction_table_t) action, ns(AllocParams_table_t) params) {
        if (eventLog_)
            eventLog_->addEvent(
                Event{
                    std::chrono::steady_clock::now(), EventType::ALLOC_START, actionIndex,
                    ns(HostAction_location_is_present(action)) ? ns(HostAction_location_get(action))
                                                               : ""
                }
            );
        ...
        LOGD << "action: Allocating buffer #" << id << ": " << ns(BufferType_name(bufferType))
        ...
        if (eventLog_)
            eventLog_->addEvent(
                Event{
                    std::chrono::steady_clock::now(), EventType::ALLOC_END, actionIndex,
                    ns(HostAction_location_is_present(action)) ? ns(HostAction_location_get(action))
                                                               : ""
                }
            );
    ...
    iree_status_t
    processDeallocAction(ns(HostAction_table_t) action, ns(DeallocParams_table_t) params) {
        if (eventLog_)
            eventLog_->addEvent(
                Event{
                    std::chrono::steady_clock::now(), EventType::DEALLOC_START, actionIndex,
                    ns(HostAction_location_is_present(action)) ? ns(HostAction_location_get(action))
                                                               : ""
                }
            );
        ...
        if (eventLog_)
            eventLog_->addEvent(
                Event{
                    std::chrono::steady_clock::now(), EventType::DEALLOC_END, actionIndex,
                    ns(HostAction_location_is_present(action)) ? ns(HostAction_location_get(action))
                                                               : ""
                }
            );
    ...
    iree_status_t processAction(ns(HostAction_table_t) action) {
        ...
        default:
            return iree_make_status(
                IREE_STATUS_UNIMPLEMENTED, "unknown host action type %d",
                ns(HostAction_params_type(action))
            );
        }
    }
};

static iree_status_t compute_xram_footprint(
    ns(ExecutableDef_table_t) executable_def, iree_hal_executable_dispatch_state_v0_t *state,
    uint32_t *xram_base, uint32_t *xram_size
) {
    ...
    // iterate over all the segments and compute the xram start and end address of each to find the
    // xram base and size
    ...
    for (size_t i = 0; i < segments_count; i++) {
        ...
        LOGD << "Segment " << i << ": xram_address=" << xram_address
             << ", xram_segment_size=" << xram_segment_size << ", xram_start=" << xram_start
             << ", xram_end=" << xram_end;
    }

    // iterate over all the bindings and compute the xram start and end address of each to find the
    // xram base and size
    ...
    for (size_t i = 0; i < bindings_count; i++) {
        ...
        LOGD << "Binding " << i << ": xram_address=" << xram_address
             << ", xram_size_binding=" << xram_size_binding << ", xram_start=" << xram_start
             << ", xram_end=" << xram_end;
    }
    ...
    for (size_t i = 0; i < actions_count; i++) {
        ...
        if (ns(HostAction_params_type(action)) == ns(HostActionParams_AllocParams)) {
            ...
            LOGD << "Alloc Action " << i << ": xram_address=" << xram_address
                 << ", xram_size_alloc=" << xram_size_alloc << ", xram_start=" << xram_start
                 << ", xram_end=" << xram_end;
        }
    }
...
iree_status_t iree_hal_torq_native_executable_run(
    ...
    iree_hal_torq_native_executable_t *executable =
        iree_hal_torq_native_executable_cast(base_value);
    iree_hal_executable_dispatch_state_v0_t *state = dispatch_state;

    LOGD << "Dispatch State:";
    LOGD << "  executable=" << base_value;
    LOGD << "  workgroup_count x=" << state->workgroup_count_x << " y=" << state->workgroup_count_y
         << " z=" << state->workgroup_count_z;
    ...
    for (iree_host_size_t i = 0; i < state->binding_count; ++i) {
        LOGD << "  binding[" << i << "]: buffer=" << state->binding_ptrs[i]
             << " length=" << state->binding_lengths[i];
    }
    ...
    LOGD << "xram_base " << xram_base << " xram_end " << (xram_base + xram_size) << " xram_size "
         << xram_size;
    LOGD << "Running executable " << executable_name;

    unique_ptr<TorqHw> torq = newTorqHw(
        FLAG_torq_hw_type, xram_base, xram_size, get_mem_dump_data_root_dir(executable_name),
        &eventLog
    );
    ...
    // simulate mapping bindings in the IO MMU of the NPU by copying back the binding buffers from
    // the cmodel
    for (size_t i = 0; i < bindings_count; i++) {
        ...
        iree_status_t ret = sync_binding(
            i, binding, state, torq.get(), false, executable_name, context.lastCompletedJobId()
        );
        if (!iree_status_is_ok(ret)) {
            return ret;
        }
