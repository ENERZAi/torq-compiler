...
namespace {
extern const iree_hal_executable_cache_vtable_t iree_hal_torq_nop_executable_cache_vtable;
} // namespace

static iree_hal_torq_nop_executable_cache_t *
iree_hal_torq_nop_executable_cache_cast(iree_hal_executable_cache_t *base_value) {
    IREE_HAL_ASSERT_TYPE(base_value, &iree_hal_torq_nop_executable_cache_vtable);
...
iree_status_t iree_hal_torq_nop_executable_cache_create(
    ...
    *out_executable_cache = NULL;
    ...
    iree_status_t status =
        iree_allocator_malloc(allocator, sizeof(*executable_cache), (void **)&executable_cache);

    if (iree_status_is_ok(status)) {
        iree_hal_resource_initialize(
            &iree_hal_torq_nop_executable_cache_vtable, &executable_cache->resource
        );
...
static void
iree_hal_torq_nop_executable_cache_destroy(iree_hal_executable_cache_t *base_executable_cache) {
...
static bool iree_hal_torq_nop_executable_cache_can_prepare_format(
    ...
    if (iree_string_view_equal(executable_format, iree_make_cstring_view("torq-fb"))) {
        return true;
    }
    return false;
}
...
static iree_status_t iree_hal_torq_nop_executable_cache_prepare_executable(
    ...
    iree_hal_torq_nop_executable_cache_t *executable_cache =
        iree_hal_torq_nop_executable_cache_cast(base_executable_cache);
    return iree_hal_torq_native_executable_create(
        executable_cache->device, executable_cache->allocator, executable_params, out_executable
    );
}

namespace {
const iree_hal_executable_cache_vtable_t iree_hal_torq_nop_executable_cache_vtable = {
