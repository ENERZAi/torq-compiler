...
static iree_status_t iree_hal_torq_driver_factory_try_create(
    ...
    if (!iree_string_view_equal(driver_name, IREE_SV("torq"))) {
        return iree_make_status(
            IREE_STATUS_UNAVAILABLE, "no driver '%.*s' is provided by this factory",
    ...
    iree_status_t status =
        iree_hal_executable_plugin_manager_create_from_flags(host_allocator, &plugin_manager);

    iree_hal_executable_loader_t *loaders[8] = {NULL};
    iree_host_size_t loader_count = 0;
    if (iree_status_is_ok(status)) {
        status = iree_hal_create_all_available_executable_loaders(
            plugin_manager, IREE_ARRAYSIZE(loaders), &loader_count, loaders, host_allocator
        );
    }
    ...
    if (iree_status_is_ok(status)) {
        status = iree_hal_torq_driver_create(
            driver_name, &default_params, loader_count, loaders, device_allocator, host_allocator,
            out_driver
        );
    }
