...
typedef struct iree_hal_torq_command_buffer_t {
    ...
    struct {
        ...
        void *full_bindings
            [IREE_HAL_LOCAL_MAX_DESCRIPTOR_SET_COUNT * IREE_HAL_LOCAL_MAX_DESCRIPTOR_BINDING_COUNT];
        size_t full_binding_lengths
            [IREE_HAL_LOCAL_MAX_DESCRIPTOR_SET_COUNT * IREE_HAL_LOCAL_MAX_DESCRIPTOR_BINDING_COUNT];
        ...
        void *packed_bindings
            [IREE_HAL_LOCAL_MAX_DESCRIPTOR_SET_COUNT * IREE_HAL_LOCAL_MAX_DESCRIPTOR_BINDING_COUNT];
        size_t packed_binding_lengths
            [IREE_HAL_LOCAL_MAX_DESCRIPTOR_SET_COUNT * IREE_HAL_LOCAL_MAX_DESCRIPTOR_BINDING_COUNT];
...
static const iree_hal_command_buffer_vtable_t iree_hal_torq_command_buffer_vtable;

static iree_hal_torq_command_buffer_t *
iree_hal_torq_command_buffer_cast(iree_hal_command_buffer_t *base_value) {
    IREE_HAL_ASSERT_TYPE(base_value, &iree_hal_torq_command_buffer_vtable);
    return (iree_hal_torq_command_buffer_t *)base_value;
}

static void iree_hal_torq_command_buffer_reset(iree_hal_torq_command_buffer_t *command_buffer) {
    memset(&command_buffer->state, 0, sizeof(command_buffer->state));

    // Setup the cached dispatch state pointers that don't change.
    iree_hal_executable_dispatch_state_v0_t *dispatch_state = &command_buffer->state.dispatch_state;
    ...
    dispatch_state->binding_lengths = command_buffer->state.packed_binding_lengths;
}

iree_host_size_t iree_hal_torq_command_buffer_size(void) {
    return sizeof(iree_hal_torq_command_buffer_t);
}
...
iree_status_t iree_hal_torq_command_buffer_initialize(
    ...
    iree_host_size_t binding_capacity, iree_allocator_t host_allocator, iree_byte_span_t storage,
    ...
    *out_command_buffer = NULL;
    ...
    if (binding_capacity > 0) {
        ...
        return iree_make_status(
            IREE_STATUS_INVALID_ARGUMENT, "indirect command buffers do not support binding tables"
        );
    }
    if (storage.data_length < iree_hal_torq_command_buffer_size()) {
        return iree_make_status(
            IREE_STATUS_INVALID_ARGUMENT, "storage must have at least the capacity as "
                                          "defined by iree_hal_torq_command_buffer_size"
        );
    }

    IREE_TRACE_ZONE_BEGIN(z0);

    iree_hal_torq_command_buffer_t *command_buffer = (iree_hal_torq_command_buffer_t *)storage.data;
    memset(command_buffer, 0, sizeof(*command_buffer));

    iree_hal_command_buffer_initialize(
        device, mode, command_categories, queue_affinity, binding_capacity,
        &iree_hal_torq_command_buffer_vtable, &command_buffer->base
    );
    command_buffer->host_allocator = host_allocator;
    iree_hal_torq_command_buffer_reset(command_buffer);
...
void iree_hal_torq_command_buffer_deinitialize(iree_hal_command_buffer_t *base_command_buffer) {
...
iree_status_t iree_hal_torq_command_buffer_create(
    ...
    *out_command_buffer = NULL;
    ...
    iree_status_t status = iree_allocator_malloc(
        host_allocator, iree_hal_torq_command_buffer_size(), (void **)&storage
    );
    iree_hal_command_buffer_t *command_buffer = NULL;
    if (iree_status_is_ok(status)) {
        status = iree_hal_torq_command_buffer_initialize(
            device, mode, command_categories, queue_affinity, binding_capacity, host_allocator,
            iree_make_byte_span(storage, iree_hal_torq_command_buffer_size()), &command_buffer
        );
    }
...
static void iree_hal_torq_command_buffer_destroy(iree_hal_command_buffer_t *base_command_buffer) {
    iree_hal_torq_command_buffer_t *command_buffer =
        iree_hal_torq_command_buffer_cast(base_command_buffer);
    iree_allocator_t host_allocator = command_buffer->host_allocator;
    IREE_TRACE_ZONE_BEGIN(z0);

    iree_hal_torq_command_buffer_deinitialize(base_command_buffer);
    iree_allocator_free(host_allocator, command_buffer);

    IREE_TRACE_ZONE_END(z0);
}

bool iree_hal_torq_command_buffer_isa(iree_hal_command_buffer_t *command_buffer) {
    return iree_hal_resource_is(&command_buffer->resource, &iree_hal_torq_command_buffer_vtable);
}

//===----------------------------------------------------------------------===//
// iree_hal_torq_command_buffer_t recording
//===----------------------------------------------------------------------===//

// Updates the cached processor ID field in the command buffer.
static void
iree_hal_torq_command_buffer_update_processor_id(iree_hal_torq_command_buffer_t *command_buffer) {
    iree_cpu_requery_processor_id(
        &command_buffer->state.processor_tag, &command_buffer->state.processor_id
    );
}

static iree_status_t
iree_hal_torq_command_buffer_begin(iree_hal_command_buffer_t *base_command_buffer) {
    iree_hal_torq_command_buffer_t *command_buffer =
        iree_hal_torq_command_buffer_cast(base_command_buffer);
    iree_hal_torq_command_buffer_reset(command_buffer);
...
static iree_status_t
iree_hal_torq_command_buffer_end(iree_hal_command_buffer_t *base_command_buffer) {
    iree_hal_torq_command_buffer_t *command_buffer =
        iree_hal_torq_command_buffer_cast(base_command_buffer);
    iree_hal_torq_command_buffer_reset(command_buffer);
    return iree_ok_status();
...
//===----------------------------------------------------------------------===//
// iree_hal_torq_command_buffer_t debug utilities
//===----------------------------------------------------------------------===//

static void iree_hal_torq_command_buffer_begin_debug_group(
...
static void
iree_hal_torq_command_buffer_end_debug_group(iree_hal_command_buffer_t *base_command_buffer) {
    // TODO(benvanik): tracy event stack.
}
...
static iree_status_t iree_hal_torq_command_buffer_execution_barrier(
...
static iree_status_t iree_hal_torq_command_buffer_signal_event(
...
static iree_status_t iree_hal_torq_command_buffer_reset_event(
    iree_hal_command_buffer_t *base_command_buffer, iree_hal_event_t *event,
    iree_hal_execution_stage_t source_stage_mask
) {
    // No-op; we execute synchronously.
...
static iree_status_t iree_hal_torq_command_buffer_wait_events(
...
static iree_status_t iree_hal_torq_command_buffer_discard_buffer(
...
static iree_status_t iree_hal_torq_command_buffer_fill_buffer(
    ...
    return iree_hal_buffer_map_fill(target_buffer, target_offset, length, pattern, pattern_length);
}
...
static iree_status_t iree_hal_torq_command_buffer_update_buffer(
    iree_hal_command_buffer_t *base_command_buffer, const void *source_buffer,
    iree_host_size_t source_offset, iree_hal_buffer_t *target_buffer,
    iree_device_size_t target_offset, iree_device_size_t length
) {
    return iree_hal_buffer_map_write(
        target_buffer, target_offset, (const uint8_t *)source_buffer + source_offset, length
    );
}
...
static iree_status_t iree_hal_torq_command_buffer_copy_buffer(
...
static iree_status_t iree_hal_torq_command_buffer_collective(
    iree_hal_command_buffer_t *base_command_buffer, iree_hal_channel_t *channel,
    iree_hal_collective_op_t op, uint32_t param, iree_hal_buffer_binding_t send_binding,
    iree_hal_buffer_binding_t recv_binding, iree_device_size_t element_count
) {
    return iree_make_status(IREE_STATUS_UNIMPLEMENTED, "collectives not yet implemented on CPU");
}
...
static iree_status_t iree_hal_torq_command_buffer_push_constants(
    ...
    if (IREE_UNLIKELY(offset + values_length >= sizeof(command_buffer->state.push_constants))) {
        return iree_make_status(
            ...
            "push constant range %" PRIhsz " (length=%" PRIhsz ") out of range", offset,
            values_length
        );
    }
...
static iree_status_t iree_hal_torq_command_buffer_push_descriptor_set(
    iree_hal_command_buffer_t *base_command_buffer, iree_hal_pipeline_layout_t *pipeline_layout,
    uint32_t set, iree_host_size_t binding_count, const iree_hal_descriptor_set_binding_t *bindings
) {
    iree_hal_torq_command_buffer_t *command_buffer =
        iree_hal_torq_command_buffer_cast(base_command_buffer);

    if (IREE_UNLIKELY(set >= IREE_HAL_LOCAL_MAX_DESCRIPTOR_SET_COUNT)) {
        return iree_make_status(IREE_STATUS_INVALID_ARGUMENT, "set %u out of bounds", set);
    }

    iree_host_size_t binding_base = set * IREE_HAL_LOCAL_MAX_DESCRIPTOR_BINDING_COUNT;
    for (iree_host_size_t i = 0; i < binding_count; ++i) {
        if (IREE_UNLIKELY(bindings[i].binding >= IREE_HAL_LOCAL_MAX_DESCRIPTOR_BINDING_COUNT)) {
            return iree_make_status(
                IREE_STATUS_INVALID_ARGUMENT, "buffer binding index out of bounds"
            );
        }
        ...
        if (bindings[i].buffer) {
            IREE_RETURN_IF_ERROR(iree_hal_buffer_map_range(
                bindings[i].buffer, IREE_HAL_MAPPING_MODE_PERSISTENT, IREE_HAL_MEMORY_ACCESS_ANY,
                bindings[i].offset, bindings[i].length, &buffer_mapping
            ));
        }
        command_buffer->state.full_bindings[binding_ordinal] = buffer_mapping.contents.data;
...
static iree_status_t iree_hal_torq_command_buffer_dispatch(
    iree_hal_command_buffer_t *base_command_buffer, iree_hal_executable_t *executable,
    int32_t entry_point, uint32_t workgroup_x, uint32_t workgroup_y, uint32_t workgroup_z
) {
    ...
    iree_hal_local_pipeline_layout_t *local_layout =
        (iree_hal_local_pipeline_layout_t *)torq_executable->pipeline_layouts[entry_point];
    ...
    iree_host_size_t used_binding_count = iree_math_count_ones_u64(used_binding_mask);
    ...
    for (iree_host_size_t i = 0; i < used_binding_count; ++i) {
        ...
        if (!binding_ptrs[i]) {
            return iree_make_status(
                IREE_STATUS_FAILED_PRECONDITION, "(flat) binding %d is NULL", binding_ordinal
            );
        }
        binding_lengths[i] = command_buffer->state.full_binding_lengths[binding_ordinal];
    }
    ...
    if (local_memory_size > 0) {
        IREE_RETURN_IF_ERROR(iree_allocator_malloc(
            command_buffer->host_allocator, local_memory_size, (void **)&local_memory.data
        ));
    }

    // Since we are running on a borrowed thread, we know nothing about the
    // floating point state. Reset it.
    iree_fpu_state_t fpu_state = iree_fpu_state_push(IREE_FPU_STATE_FLAG_FLUSH_DENORMALS_TO_ZERO);
...
static iree_status_t iree_hal_torq_command_buffer_dispatch_indirect(
    iree_hal_command_buffer_t *base_command_buffer, iree_hal_executable_t *executable,
    int32_t entry_point, iree_hal_buffer_t *workgroups_buffer, iree_device_size_t workgroups_offset
) {
    if (true) {
        return iree_make_status(IREE_STATUS_UNIMPLEMENTED, "indirect dispatch not yet implemented");
    }
    // TODO(benvanik): track mapping so we can properly map/unmap/flush/etc.
    iree_hal_buffer_mapping_t buffer_mapping = {{0}};
    IREE_RETURN_IF_ERROR(iree_hal_buffer_map_range(
        workgroups_buffer, IREE_HAL_MAPPING_MODE_PERSISTENT, IREE_HAL_MEMORY_ACCESS_READ,
        workgroups_offset, 3 * sizeof(uint32_t), &buffer_mapping
    ));
    iree_hal_vec3_t workgroup_count = *(const iree_hal_vec3_t *)buffer_mapping.contents.data;
    return iree_hal_torq_command_buffer_dispatch(
        base_command_buffer, executable, entry_point, workgroup_count.x, workgroup_count.y,
        workgroup_count.z
    );
}
...
static iree_status_t iree_hal_torq_command_buffer_execute_commands(
    ...
    return iree_make_status(
        IREE_STATUS_UNIMPLEMENTED, "execute command buffers not yet implemented"
    );
}
...
static const iree_hal_command_buffer_vtable_t iree_hal_torq_command_buffer_vtable = {
    .destroy = iree_hal_torq_command_buffer_destroy,
    .begin = iree_hal_torq_command_buffer_begin,
    .end = iree_hal_torq_command_buffer_end,
    .begin_debug_group = iree_hal_torq_command_buffer_begin_debug_group,
    .end_debug_group = iree_hal_torq_command_buffer_end_debug_group,
    .execution_barrier = iree_hal_torq_command_buffer_execution_barrier,
    .signal_event = iree_hal_torq_command_buffer_signal_event,
    .reset_event = iree_hal_torq_command_buffer_reset_event,
    .wait_events = iree_hal_torq_command_buffer_wait_events,
    .discard_buffer = iree_hal_torq_command_buffer_discard_buffer,
    .fill_buffer = iree_hal_torq_command_buffer_fill_buffer,
    .update_buffer = iree_hal_torq_command_buffer_update_buffer,
    .copy_buffer = iree_hal_torq_command_buffer_copy_buffer,
    .collective = iree_hal_torq_command_buffer_collective,
    .push_constants = iree_hal_torq_command_buffer_push_constants,
    .push_descriptor_set = iree_hal_torq_command_buffer_push_descriptor_set,
    .dispatch = iree_hal_torq_command_buffer_dispatch,
    .dispatch_indirect = iree_hal_torq_command_buffer_dispatch_indirect,
    .execute_commands = iree_hal_torq_command_buffer_execute_commands,
};
