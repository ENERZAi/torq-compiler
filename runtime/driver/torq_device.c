...
typedef struct iree_hal_torq_device_t {
...
} iree_hal_torq_device_t;

static const iree_hal_device_vtable_t iree_hal_torq_device_vtable;

static iree_hal_torq_device_t *iree_hal_torq_device_cast(iree_hal_device_t *base_value) {
    IREE_HAL_ASSERT_TYPE(base_value, &iree_hal_torq_device_vtable);
    return (iree_hal_torq_device_t *)base_value;
}

void iree_hal_torq_device_params_initialize(iree_hal_torq_device_params_t *out_params) {
    memset(out_params, 0, sizeof(*out_params));
    out_params->arena_block_size = 32 * 1024;
}

static iree_status_t
iree_hal_torq_device_check_params(const iree_hal_torq_device_params_t *params) {
    if (params->arena_block_size < 4096) {
        return iree_make_status(
            IREE_STATUS_INVALID_ARGUMENT, "arena block size too small (< 4096 bytes)"
        );
    }
    return iree_ok_status();
}

iree_status_t iree_hal_torq_device_create(
    iree_string_view_t identifier, const iree_hal_torq_device_params_t *params,
    ...
    *out_device = NULL;
    ...
    IREE_RETURN_AND_END_ZONE_IF_ERROR(z0, iree_hal_torq_device_check_params(params));

    iree_hal_torq_device_t *device = NULL;
    iree_host_size_t struct_size = sizeof(*device) + loader_count * sizeof(*device->loaders);
    iree_host_size_t total_size = struct_size + identifier.size;
    iree_status_t status = iree_allocator_malloc(host_allocator, total_size, (void **)&device);
    if (iree_status_is_ok(status)) {
        memset(device, 0, total_size);
        iree_hal_resource_initialize(&iree_hal_torq_device_vtable, &device->resource);
        iree_string_view_append_to_buffer(
            identifier, &device->identifier, (char *)device + struct_size
        );
        device->host_allocator = host_allocator;
        device->device_allocator = device_allocator;
        iree_hal_allocator_retain(device_allocator);
        iree_arena_block_pool_initialize(
            params->arena_block_size, host_allocator, &device->large_block_pool
        );
...
static void iree_hal_torq_device_destroy(iree_hal_device_t *base_device) {
...
static iree_string_view_t iree_hal_torq_device_id(iree_hal_device_t *base_device) {
    iree_hal_torq_device_t *device = iree_hal_torq_device_cast(base_device);
    return device->identifier;
}

static iree_allocator_t iree_hal_torq_device_host_allocator(iree_hal_device_t *base_device) {
    iree_hal_torq_device_t *device = iree_hal_torq_device_cast(base_device);
    return device->host_allocator;
}

static iree_hal_allocator_t *iree_hal_torq_device_allocator(iree_hal_device_t *base_device) {
    iree_hal_torq_device_t *device = iree_hal_torq_device_cast(base_device);
    return device->device_allocator;
}

static void iree_hal_torq_replace_device_allocator(
...
static void iree_hal_torq_replace_channel_provider(
...
static iree_status_t iree_hal_torq_device_trim(iree_hal_device_t *base_device) {
    iree_hal_torq_device_t *device = iree_hal_torq_device_cast(base_device);
    return iree_hal_allocator_trim(device->device_allocator);
}

static iree_status_t iree_hal_torq_device_query_i64(
    iree_hal_device_t *base_device, iree_string_view_t category, iree_string_view_t key,
    int64_t *out_value
) {
    iree_hal_torq_device_t *device = iree_hal_torq_device_cast(base_device);
    *out_value = 0;
    ...
    if (iree_string_view_equal(category, IREE_SV("hal.executable.format"))) {
        ...
        if (iree_string_view_equal(key, IREE_SV("torq-fb"))) {
            *out_value = 1;
        }
    ...
    return iree_make_status(
        IREE_STATUS_NOT_FOUND, "unknown device configuration key value '%.*s :: %.*s'",
        (int)category.size, category.data, (int)key.size, key.data
    );
}

static iree_status_t iree_hal_torq_device_create_channel(
...
static iree_status_t iree_hal_torq_device_create_command_buffer(
    ...
    return iree_hal_torq_command_buffer_create(
...
static iree_status_t iree_hal_torq_device_create_descriptor_set_layout(
    ...
    return iree_hal_local_descriptor_set_layout_create(
        flags, binding_count, bindings, iree_hal_device_host_allocator(base_device),
        out_descriptor_set_layout
    );
}

static iree_status_t
iree_hal_torq_device_create_event(iree_hal_device_t *base_device, iree_hal_event_t **out_event) {
    return iree_hal_torq_event_create(iree_hal_device_host_allocator(base_device), out_event);
}

static iree_status_t iree_hal_torq_device_create_executable_cache(
    iree_hal_device_t *base_device, iree_string_view_t identifier, iree_loop_t loop,
    iree_hal_executable_cache_t **out_executable_cache
) {
    return iree_hal_torq_nop_executable_cache_create(
        base_device, identifier, iree_hal_device_host_allocator(base_device), out_executable_cache
    );
}

static iree_status_t iree_hal_torq_device_import_file(
    iree_hal_device_t *base_device, iree_hal_queue_affinity_t queue_affinity,
    ...
    if (iree_io_file_handle_type(handle) != IREE_IO_FILE_HANDLE_TYPE_HOST_ALLOCATION) {
        return iree_make_status(
            IREE_STATUS_UNAVAILABLE, "implementation does not support the external file type"
        );
    }
...
static iree_status_t iree_hal_torq_device_create_pipeline_layout(
    ...
    return iree_hal_local_pipeline_layout_create(
        push_constants, set_layout_count, set_layouts, iree_hal_device_host_allocator(base_device),
        out_pipeline_layout
    );
}

static iree_status_t iree_hal_torq_device_create_semaphore(
    iree_hal_device_t *base_device, uint64_t initial_value, iree_hal_semaphore_t **out_semaphore
) {
    iree_hal_torq_device_t *device = iree_hal_torq_device_cast(base_device);
    return iree_hal_torq_semaphore_create(
        &device->semaphore_state, initial_value, device->host_allocator, out_semaphore
    );
}

static iree_hal_semaphore_compatibility_t iree_hal_torq_device_query_semaphore_compatibility(
...
static iree_status_t iree_hal_torq_device_queue_alloca(
    iree_hal_device_t *base_device, iree_hal_queue_affinity_t queue_affinity,
    ...
    IREE_RETURN_IF_ERROR(iree_hal_allocator_allocate_buffer(
        iree_hal_device_allocator(base_device), params, allocation_size, out_buffer
    ));
    IREE_RETURN_IF_ERROR(iree_hal_semaphore_list_signal(signal_semaphore_list));
    return iree_ok_status();
}

static iree_status_t iree_hal_torq_device_queue_dealloca(
    iree_hal_device_t *base_device, iree_hal_queue_affinity_t queue_affinity,
    const iree_hal_semaphore_list_t wait_semaphore_list,
...
static iree_status_t iree_hal_torq_device_apply_deferred_command_buffers(
    iree_hal_torq_device_t *device, iree_host_size_t command_buffer_count,
    ...
    iree_byte_span_t storage = iree_make_byte_span(
        iree_alloca(iree_hal_inline_command_buffer_size()), iree_hal_inline_command_buffer_size()
    );
    ...
    for (iree_host_size_t i = 0; i < command_buffer_count; ++i) {
        ...
        if (iree_hal_deferred_command_buffer_isa(command_buffer)) {
            ...
            IREE_RETURN_IF_ERROR(iree_hal_inline_command_buffer_initialize(
                ...
                /*binding_capacity=*/0, device->host_allocator, storage, &inline_command_buffer
            ));
            iree_status_t status = iree_hal_deferred_command_buffer_apply(
                command_buffer, inline_command_buffer, iree_hal_buffer_binding_table_empty()
            );
...
static iree_status_t iree_hal_torq_device_queue_read(
    iree_hal_device_t *base_device, iree_hal_queue_affinity_t queue_affinity,
    const iree_hal_semaphore_list_t wait_semaphore_list,
...
static iree_status_t iree_hal_torq_device_queue_write(
    iree_hal_device_t *base_device, iree_hal_queue_affinity_t queue_affinity,
    const iree_hal_semaphore_list_t wait_semaphore_list,
...
static iree_status_t iree_hal_torq_device_queue_execute(
    iree_hal_device_t *base_device, iree_hal_queue_affinity_t queue_affinity,
    const iree_hal_semaphore_list_t wait_semaphore_list,
    ...
    IREE_RETURN_IF_ERROR(
        iree_hal_torq_semaphore_multi_signal(&device->semaphore_state, signal_semaphore_list)
    );
...
static iree_status_t iree_hal_torq_device_wait_semaphores(
    iree_hal_device_t *base_device, iree_hal_wait_mode_t wait_mode,
    const iree_hal_semaphore_list_t semaphore_list, iree_timeout_t timeout
) {
    iree_hal_torq_device_t *device = iree_hal_torq_device_cast(base_device);
    return iree_hal_torq_semaphore_multi_wait(
        &device->semaphore_state, wait_mode, semaphore_list, timeout
    );
}
...
static iree_status_t iree_hal_torq_device_profiling_flush(iree_hal_device_t *base_device) {
    // Unimplemented (and that's ok).
    return iree_ok_status();
}

static iree_status_t iree_hal_torq_device_profiling_end(iree_hal_device_t *base_device) {
    // Unimplemented (and that's ok).
    return iree_ok_status();
}

static const iree_hal_device_vtable_t iree_hal_torq_device_vtable = {
    .destroy = iree_hal_torq_device_destroy,
    .id = iree_hal_torq_device_id,
    .host_allocator = iree_hal_torq_device_host_allocator,
    .device_allocator = iree_hal_torq_device_allocator,
    .replace_device_allocator = iree_hal_torq_replace_device_allocator,
    .replace_channel_provider = iree_hal_torq_replace_channel_provider,
    .trim = iree_hal_torq_device_trim,
    .query_i64 = iree_hal_torq_device_query_i64,
    .create_channel = iree_hal_torq_device_create_channel,
    .create_command_buffer = iree_hal_torq_device_create_command_buffer,
    .create_descriptor_set_layout = iree_hal_torq_device_create_descriptor_set_layout,
    .create_event = iree_hal_torq_device_create_event,
    .create_executable_cache = iree_hal_torq_device_create_executable_cache,
    .import_file = iree_hal_torq_device_import_file,
    .create_pipeline_layout = iree_hal_torq_device_create_pipeline_layout,
    .create_semaphore = iree_hal_torq_device_create_semaphore,
    .query_semaphore_compatibility = iree_hal_torq_device_query_semaphore_compatibility,
    .queue_alloca = iree_hal_torq_device_queue_alloca,
    .queue_dealloca = iree_hal_torq_device_queue_dealloca,
    .queue_read = iree_hal_torq_device_queue_read,
    .queue_write = iree_hal_torq_device_queue_write,
    .queue_execute = iree_hal_torq_device_queue_execute,
    .queue_flush = iree_hal_torq_device_queue_flush,
    .wait_semaphores = iree_hal_torq_device_wait_semaphores,
    .profiling_begin = iree_hal_torq_device_profiling_begin,
    .profiling_flush = iree_hal_torq_device_profiling_flush,
    .profiling_end = iree_hal_torq_device_profiling_end,
};
