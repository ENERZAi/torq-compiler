...
//===----------------------------------------------------------------------===//
// iree_hal_torq_semaphore_state_t
//===----------------------------------------------------------------------===//

void iree_hal_torq_semaphore_state_initialize(iree_hal_torq_semaphore_state_t *out_shared_state) {
    memset(out_shared_state, 0, sizeof(*out_shared_state));
    iree_notification_initialize(&out_shared_state->notification);
}

void iree_hal_torq_semaphore_state_deinitialize(iree_hal_torq_semaphore_state_t *shared_state) {
    iree_notification_deinitialize(&shared_state->notification);
    memset(shared_state, 0, sizeof(*shared_state));
}

//===----------------------------------------------------------------------===//
// iree_hal_torq_semaphore_t
//===----------------------------------------------------------------------===//

typedef struct iree_hal_torq_semaphore_t {
    iree_hal_semaphore_t base;
    iree_allocator_t host_allocator;

    // Shared across all semaphores.
    iree_hal_torq_semaphore_state_t *shared_state;
...
static const iree_hal_semaphore_vtable_t iree_hal_torq_semaphore_vtable;

static iree_hal_torq_semaphore_t *iree_hal_torq_semaphore_cast(iree_hal_semaphore_t *base_value) {
    IREE_HAL_ASSERT_TYPE(base_value, &iree_hal_torq_semaphore_vtable);
    return (iree_hal_torq_semaphore_t *)base_value;
}

iree_status_t iree_hal_torq_semaphore_create(
    iree_hal_torq_semaphore_state_t *shared_state, uint64_t initial_value,
    ...
    *out_semaphore = NULL;
    ...
    iree_hal_torq_semaphore_t *semaphore = NULL;
    iree_status_t status =
        iree_allocator_malloc(host_allocator, sizeof(*semaphore), (void **)&semaphore);
    if (iree_status_is_ok(status)) {
        iree_hal_semaphore_initialize(&iree_hal_torq_semaphore_vtable, &semaphore->base);
...
static void iree_hal_torq_semaphore_destroy(iree_hal_semaphore_t *base_semaphore) {
    iree_hal_torq_semaphore_t *semaphore = iree_hal_torq_semaphore_cast(base_semaphore);
...
iree_hal_torq_semaphore_query(iree_hal_semaphore_t *base_semaphore, uint64_t *out_value) {
    iree_hal_torq_semaphore_t *semaphore = iree_hal_torq_semaphore_cast(base_semaphore);
...
static iree_status_t
iree_hal_torq_semaphore_signal_unsafe(iree_hal_torq_semaphore_t *semaphore, uint64_t new_value) {
...
static iree_status_t
iree_hal_torq_semaphore_signal(iree_hal_semaphore_t *base_semaphore, uint64_t new_value) {
    iree_hal_torq_semaphore_t *semaphore = iree_hal_torq_semaphore_cast(base_semaphore);

    iree_slim_mutex_lock(&semaphore->mutex);

    iree_status_t status = iree_hal_torq_semaphore_signal_unsafe(semaphore, new_value);
    if (!iree_status_is_ok(status)) {
        iree_slim_mutex_unlock(&semaphore->mutex);
        return status;
    }
    ...
    iree_notification_post(&semaphore->shared_state->notification, IREE_ALL_WAITERS);

    return iree_ok_status();
}

static void
iree_hal_torq_semaphore_fail(iree_hal_semaphore_t *base_semaphore, iree_status_t status) {
    iree_hal_torq_semaphore_t *semaphore = iree_hal_torq_semaphore_cast(base_semaphore);
    ...
    iree_hal_semaphore_notify(&semaphore->base, IREE_HAL_SEMAPHORE_FAILURE_VALUE, status_code);

    iree_notification_post(&semaphore->shared_state->notification, IREE_ALL_WAITERS);
}

iree_status_t iree_hal_torq_semaphore_multi_signal(
    iree_hal_torq_semaphore_state_t *shared_state, const iree_hal_semaphore_list_t semaphore_list
) {
    IREE_ASSERT_ARGUMENT(shared_state);
    if (semaphore_list.count == 0) {
        ...
        return iree_hal_semaphore_signal(
    ...
    for (iree_host_size_t i = 0; i < semaphore_list.count; ++i) {
        iree_hal_torq_semaphore_t *semaphore =
            iree_hal_torq_semaphore_cast(semaphore_list.semaphores[i]);

        iree_slim_mutex_lock(&semaphore->mutex);
        status = iree_hal_torq_semaphore_signal_unsafe(semaphore, semaphore_list.payload_values[i]);
        if (!iree_status_is_ok(status)) {
            iree_slim_mutex_unlock(&semaphore->mutex);
            break;
        }
        ...
        iree_hal_semaphore_notify(
            semaphore_list.semaphores[i], semaphore_list.payload_values[i], IREE_STATUS_OK
        );
    }
...
typedef struct iree_hal_torq_semaphore_notify_state_t {
    iree_hal_torq_semaphore_t *semaphore;
    uint64_t value;
} iree_hal_torq_semaphore_notify_state_t;

static bool iree_hal_torq_semaphore_is_signaled(iree_hal_torq_semaphore_notify_state_t *state) {
    iree_hal_torq_semaphore_t *semaphore = state->semaphore;
    iree_slim_mutex_lock(&semaphore->mutex);
    bool is_signaled =
        semaphore->current_value >= state->value || !iree_status_is_ok(semaphore->failure_status);
    iree_slim_mutex_unlock(&semaphore->mutex);
    return is_signaled;
}

static iree_status_t iree_hal_torq_semaphore_wait(
    ...
    iree_hal_torq_semaphore_state_t *shared_state = semaphore->shared_state;
    iree_hal_torq_semaphore_notify_state_t notify_state = {
        .semaphore = semaphore,
        .value = value,
    };
    iree_notification_await(
        &shared_state->notification, (iree_condition_fn_t)iree_hal_torq_semaphore_is_signaled,
...
static bool iree_hal_torq_semaphore_any_signaled(const iree_hal_semaphore_list_t *semaphore_list) {
    for (iree_host_size_t i = 0; i < semaphore_list->count; ++i) {
        iree_hal_torq_semaphore_t *semaphore =
            iree_hal_torq_semaphore_cast(semaphore_list->semaphores[i]);
        iree_slim_mutex_lock(&semaphore->mutex);
        bool is_signaled = semaphore->current_value >= semaphore_list->payload_values[i] ||
...
// Used with with iree_condition_fn_t and must match that signature.
static bool iree_hal_torq_semaphore_all_signaled(const iree_hal_semaphore_list_t *semaphore_list) {
    for (iree_host_size_t i = 0; i < semaphore_list->count; ++i) {
        iree_hal_torq_semaphore_t *semaphore =
            iree_hal_torq_semaphore_cast(semaphore_list->semaphores[i]);
        iree_slim_mutex_lock(&semaphore->mutex);
...
static iree_status_t iree_hal_torq_semaphore_result_from_state(
    ...
    for (iree_host_size_t i = 0; i < semaphore_list.count; ++i) {
        ...
        const iree_status_code_t current_status_code = iree_status_code(semaphore->failure_status);
...
iree_status_t iree_hal_torq_semaphore_multi_wait(
    iree_hal_torq_semaphore_state_t *shared_state, iree_hal_wait_mode_t wait_mode,
    ...
    if (semaphore_list.count == 0) {
        ...
        return iree_hal_semaphore_wait(
            semaphore_list.semaphores[0], semaphore_list.payload_values[0], timeout
        );
    }
    ...
    if (iree_timeout_is_immediate(timeout)) {
        iree_status_t status = iree_hal_torq_semaphore_result_from_state(wait_mode, semaphore_list);
        IREE_TRACE_ZONE_END(z0);
        return status;
    }
    ...
    iree_notification_await(
        ...
        wait_mode == IREE_HAL_WAIT_MODE_ALL
            ? (iree_condition_fn_t)iree_hal_torq_semaphore_all_signaled
            : (iree_condition_fn_t)iree_hal_torq_semaphore_any_signaled,
...
static const iree_hal_semaphore_vtable_t iree_hal_torq_semaphore_vtable = {
    .destroy = iree_hal_torq_semaphore_destroy,
    .query = iree_hal_torq_semaphore_query,
    .signal = iree_hal_torq_semaphore_signal,
    .fail = iree_hal_torq_semaphore_fail,
    .wait = iree_hal_torq_semaphore_wait,
};
