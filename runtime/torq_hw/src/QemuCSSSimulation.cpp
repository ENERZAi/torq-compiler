...
IREE_FLAG(
    int32_t, torq_qemu_debug, 0,
    "Enable gdb when invoking qemu for CSS emulation (listens on the specified TCP port)"
)
IREE_FLAG(
    bool, torq_qemu_trace_instructions, false,
    "Enable qemu instruction tracing (output in /tmp/qemu_trace_XXXX)"
)
...
static void prepare_code(void *cpu, char *code_file) {
    ...
    save_memory_to_temporary_file(
        host_code_addr, REG_SIZE__TORQ_CV_ITCM - reg_code_addr, code_file
    );
    LOG("Saved code to %s\n", code_file);
}
...
static void
run_cpu_qemu_binary_ex(void *cpu, int debugger_port, bool trace_instructions, bool re_run);
...
run_cpu_qemu_binary_ex(void *cpu, int debugger_port, bool trace_instructions, bool re_run) {
    ...
    qemu_cmd +=
        "qemu-system-riscv32 -cpu rv32,v=true,vlen=128,elen=32 -nographic -M virt,memory-backend=" +
        memory_id + " -m 10M";
    qemu_cmd += " -bios " + std::string(code_file);
    qemu_cmd += " -object memory-backend-file,size=10M,id=" + memory_id +
                ",mem-path=" + std::string(memory_file) + ",share=on,prealloc=on";
